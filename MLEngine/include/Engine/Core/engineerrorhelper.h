...
public:
    ...
    inline static bool isErrorVk(const EngineError error){
        return (error <= EngineError::VK_Incomplete && error >= EngineError::VK_NotReady) || (error <= EngineError::VK_ErrorOutOfHostMemory && error >= EngineError::VK_FailedCreateBuffer);
    }
